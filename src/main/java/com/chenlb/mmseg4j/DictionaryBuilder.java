...
public class DictionaryBuilder {
	...
	private static final ConcurrentHashMap<File, DictionaryBuilder> dics = new ConcurrentHashMap<File, DictionaryBuilder>();
	 ...
	 */
	public static DictionaryBuilder clear(String path) {
	 ...
	 * @return 没有返回 null
	 */
	public static DictionaryBuilder clear(File path) {
	 ...
	 * @param path 词典的目录
	 */
	public static DictionaryBuilder getInstance(String path) {
		return getInstance(new File(path));
	}
	/**
	 * @param path 词典的目录
	 */
	public static DictionaryBuilder getInstance(File path) {
		log.info("try to load dir="+path);
		File normalizeDir = normalizeFile(path);
		DictionaryBuilder dic = dics.get(normalizeDir);
		if(dic == null) {
			dic = new DictionaryBuilder(normalizeDir);
	 ...
	 * 词典的目录
	 */
	private DictionaryBuilder(File path) {
		dicPath = path;
	 ...
	 * @return
	 */
	public synchronized Dictionary createDic() {
		Map<Character, CharNode> newDict = null;
		Map<Character, Object> newUnit = null;
		try {
			newDict = loadDic(dicPath);
			newUnit = loadUnit(dicPath);
		...
		Dictionary dic = new Dictionary(newDict, newUnit);
		return dic;		
	}
